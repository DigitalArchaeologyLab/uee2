...
class Reference(models.Model):
    ...
    publication_info = RichTextField(
        "Publication information",
        help_text="Enter the rest of the reference citation, following the bibliographic style used by the author (i.e. Form, fabric and function: Some observations on the cooking pot in antiquity, In Technology and style 2, Ceramics and Civilization. ed. W. David Kingery , pp. 157 - 172. Columbus: American Ceramic Society.",
    )
    url = models.URLField(
        "Persistent URL",
        null=True,
        blank=True,
        help_text="If a DOI or other type of persistent identifier is available, add the DOI label here (i.e. doi:0000000/000000000000)",
    )
    article = models.ManyToManyField(
        "Article",
        blank=True,
        help_text="Select the article(s) this resource is referenced in - if an article is already selected, be sure to hold Control or Command when selecting an additional article.",
    )
